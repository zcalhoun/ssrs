...
class UNet(base_model.Base):
    ...
    def __init__(
        ...
        if self.encoder_name == "base":
            ...
            self.encoder = encoders.models(
                self.encoder_name, pretrained, (2, 2, 2, 2, 2), True
            )
            ...
            conv_chan = [
                d_in // 2 + d_out
                for (d_in, d_out) in zip(self.decode_in_chans, self.decode_out_chans)
            ]
            pad = 1
        ...
        if self.use_ocr:
            self.encoder.ocr = ocr.OCRModule(
                self.n_class, *self.encoder.chans[:2][::-1], self.encoder.chans[0]
            )
        self.decoder = UnetDecoder(
            self.decode_in_chans,
            self.decode_out_chans,
            self.margins,
            self.n_class,
            conv_chan,
            pad,
    ...
    def forward(self, x):
        ...
        if self.use_ocr:
            ...
            output_dict["region"] = F.interpolate(
                region, size=input_size, mode="bilinear", align_corners=False
            )
        if self.aux_loss:
            output_dict["aux"] = self.cls(
                F.adaptive_max_pool2d(input=ftr, output_size=(1, 1)).view(
                    -1, ftr.size(1)
                )
            )
